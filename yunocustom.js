      ...
      if(!ReportChannel){
        try{
            ReportChannel = await m.guild.channels.create('antispam-logs', {
              ...
              .then(m=> m.send(`A channel was created to check muted logs!`))
              .catch(console.error)
      ...
      if (!role) {
        try {
            role = await m.guild.roles.create({
              ...
              reason: `Made for mutting people!`
            })
           ...
           ReportChannel.send(`Role **\`Muted\`** created `) 
        } catch (e) {
            console.log(e.stack);
        }
      ...
      if (user) {
        user.roles.add(role).then(()=>{
          ...
          let muteEmbed = new MessageEmbed()
            ...
            .addField(`Muted for:`,`${timeMuted} seconds`)
            .addField('Reason: ', `Spam`)
            .addField(`Time muted :`,TheDate+ " at "+ clock+" "+amORpm)
            .setColor('#D9D900')
          ...
          m.guild.owner.send(`I do not have sufficient perms to mute <@!${message.author.id}>! `);
          return false;
      });
    }//end of user
  }
